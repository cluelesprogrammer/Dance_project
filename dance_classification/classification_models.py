...
c3d_path = os.path.join(os.getcwd(), 'models/C3D_model/C3D_model.py')
i3d_path = os.path.join(os.getcwd(), 'models/pytorch-i3d/pytorch_i3d.py')
C3D_lib = imp.load_source('C3D_model', c3d_path)
i3d_lib = imp.load_source('pytorch-i3d', i3d_path)
"""
to adjust the dimension of the final classifier layer.
number of flow every n frames is the key while the value is
the value that the number of class types needs to be multiplied to, to
obtain the input shape of classifier layer
"""
flow_classifier_dict = {1:15, 2:11, 3:10, 4:9, 6:9, 8:8, 16:8}

class n_stage_bodypose_model(nn.Module):
	def __init__(self, n_layers):
		super(n_stage_bodypose_model, self).__init__()
		self.bodypose_model = bodypose_model() #loads automatically trained model
		self.n_layers, self.upsample = n_layers, nn.Upsample(scale_factor=8)
		self.ftrs, self.paf_models, self.conf_models = self.bodypose_model.features_extractor, self.bodypose_model.paf_models[0:n_layers], self.bodypose_model.conf_models[0:n_layers]
	def forward(self, x):
		x = self.ftrs[0](x)
		ftrs = x.detach()
		for i in range(self.n_layers):
			paf_out = self.paf_models[i](x)
			conf_out = self.conf_models[i](x)
			x = torch.cat([paf_out, conf_out, ftrs], 1)
		conf_out, paf_out = self.upsample(conf_out),self.upsample(paf_out)
		return paf_out, conf_out
	def max_pafs(self,x):
...
class Bodypose_Classifier(nn.Module):
	def __init__(self, dance_types=16):
		super(Bodypose_Classifier, self).__init__()
		self.pose_i3d = Bodypose_i3d()
		self.conv3d = nn.Conv3d(1024, dance_types, kernel_size=[1,1,1], stride=[1,1,1])
		self.avg_pool = nn.AvgPool3d(kernel_size=[2, 7, 7], stride=(1, 1, 1))
		self.flatten = nn.Flatten()
		self.relu = nn.ReLU()
		self.classifier = nn.Linear(7 * dance_types, dance_types)
	def forward(self, x):
		flow_ftrs = self.pose_i3d(x)
		x = self.conv3d(flow_ftrs)
		#x = self.relu1(x)
		x = self.avg_pool(x)
		x = self.flatten(x)
		x = self.classifier(x)
...
class Flow_Classifier(nn.Module):
	def __init__(self, dance_types=16, train_features=False, flow_every=1):
		super(Flow_i3d, self).__init__()
		self.flow_i3d = i3d_lib.InceptionI3d(num_classes=157, in_channels=2)
		flow_weights = torch.load('models/pytorch-i3d/models/flow_charades.pt', map_location=torch.device('cpu'))
		self.flow_i3d.load_state_dict(flow_weights)
		self.conv3d = nn.Conv3d(1024, dance_types, kernel_size=[1,1,1], stride=[1,1,1])
		self.avg_pool = nn.AvgPool3d(kernel_size=[2, 7, 7], stride=(1, 1, 1))
		self.flatten = nn.Flatten()
		self.relu = nn.ReLU()
		self.classifier = nn.Linear(flow_classifier_dict[flow_every] * dance_types, dance_types)
	def forward(self, x):
		flow_ftrs = self.flow_i3d.extract_features(pose_ftrs, 'Mixed_5c')
		x = self.conv3d(flow_ftrs)
		x = self.avg_pool(x)
		x = self.flatten(x)
		x = self.classifier(x)
...
class Bodypose_i3d(nn.Module):
	def __init__(self, dance_types=16):
		super(Bodypose_i3d, self).__init__()
		self.flow_i3d = i3d_lib.InceptionI3d(num_classes=157, in_channels=2)
		flow_weights = torch.load('models/pytorch-i3d/models/flow_charades.pt', map_location=torch.device('cpu'))
		self.flow_i3d.load_state_dict(flow_weights)
	def forward(self, x):
		flow_ftrs = self.pose_i3d.extract_features(pose_ftrs, 'Mixed_5c')
		return pose_ftrs
...
class Bodypose_i3d(nn.Module):
	def __init__(self, dance_types=16):
		super(Bodypose_i3d, self).__init__()
		self.pose_i3d = i3d_lib.InceptionI3d(num_classes=157, in_channels=2)
		flow_weights = torch.load('models/pytorch-i3d/models/flow_charades.pt', map_location=torch.device('cpu'))
		self.pose_i3d.load_state_dict(flow_weights)
		self.pose_extractor = n_stage_bodypose_model(3)
	def forward(self, x):
		pose_input = x.permute(0, 2, 1, 3, 4)
		pose_ftrs = []
		for p in pose_input:
			y = self.pose_extractor.max_pafs(p)
			pose_ftrs.append(y)
		pose_ftrs = torch.stack(pose_ftrs)
		pose_ftrs = pose_ftrs.permute(0, 2, 1, 3, 4)
		pose_ftrs = self.pose_i3d.extract_features(pose_ftrs, 'Mixed_5c')
		return pose_ftrs


class Flow_Bodypose_Classifier(nn.Module):
	def __init__(self, dance_types=16, train_flow=False, flow_every=1):
		super(Flow_Bodypose_Classifier, self).__init__()
		self.flow_i3d = i3d_lib.InceptionI3d(num_classes=157, in_channels=2)
		flow_weights = torch.load('models/pytorch-i3d/models/flow_charades.pt', map_location=torch.device('cpu'))
		self.flow_i3d.load_state_dict(flow_weights)
		self.pose_i3d = Bodypose_i3d()
		if (train_flow == False):
			for p in self.flow_i3d.parameters():
	...
	def forward(self,x1,x2):
		flow_ftrs =  self.flow_i3d.extract_features(x2, 'Mixed_5c')
		pose_ftrs = self.pose_i3d(x1)
		ftrs = torch.cat((flow_ftrs, pose_ftrs), 2)
		x = self.conv3d(ftrs)
		x = self.relu(x)
		#x = self.relu1(x)
		x = self.avg_pool(x)
...
class RGB_Bodypose_Classifier(nn.Module):
	def __init__(self, dance_types=16, train_features=(False, False)):
		super(RGB_Bodypose_Classifier, self).__init__()
		self.rgb_i3d, self.pose_i3d = i3d_lib.InceptionI3d(num_classes=157), Bodypose_i3d()
		rgb_weights = torch.load('models/pytorch-i3d/models/rgb_charades.pt', map_location=torch.device('cpu'))
		self.rgb_i3d.load_state_dict(rgb_weights)
		self.train_features = train_features
		self.dance_types = dance_types
		if (train_features[0] == False):
			for p in self.rgb_i3d.parameters():
				p.requires_grad = False
		self.conv3d = nn.Conv3d(1024, dance_types, kernel_size=[1,1,1], stride=[1,1,1])
		self.avg_pool = nn.AvgPool3d(kernel_size=[2, 7, 7], stride=(1, 1, 1))
		self.flatten = nn.Flatten()
		...
		self.classifier = nn.Linear(7 * dance_types, dance_types)
	def forward(self,x):
		rgb_ftrs =  self.rgb_i3d.extract_features(x, 'Mixed_5c')
		pose_ftrs = self.pose_i3d(x)
		ftrs = torch.cat((rgb_ftrs, pose_ftrs), 1)
		x = self.conv3d(ftrs)
		#x = self.relu1(x)
		x = self.avg_pool(x)
		x = self.flatten(x)
...
class RGB_Flow_Classifier(nn.Module):
	def __init__(self, dance_types=16, train_features=(False, True), flow_every=1):
		super(RGB_Flow_Classifier, self).__init__()
		self.rgb_i3d, self.flow_i3d = i3d_lib.InceptionI3d(num_classes=157), i3d_lib.InceptionI3d(num_classes=157, in_channels=2)
		rgb_weights, flow_weights = torch.load('models/pytorch-i3d/models/rgb_charades.pt'), torch.load('models/pytorch-i3d/models/flow_charades.pt')
		self.rgb_i3d.load_state_dict(rgb_weights)
		self.flow_i3d.load_state_dict(flow_weights)
		self.train_features = train_features
		self.dance_types = dance_types
		for p in self.rgb_i3d.parameters():
			p.requires_grad = train_features[0]
		for p in self.flow_i3d.parameters():
			p.requires_grad= train_features[1]
		...
		self.classifier = nn.Linear(flow_classifier_dict[flow_every] * dance_types, dance_types)
	def forward(self, x1, x2):
		image_ftrs, flow_ftrs = self.rgb_i3d.extract_features(x1, 'Mixed_5c'), self.flow_i3d.extract_features(x2, 'Mixed_5c')
		x = torch.cat((image_ftrs, flow_ftrs), 2)
		x = self.conv3d(x)
		x = self.relu(x)
		x = self.avg_pool(x)
		x = self.flatten(x)
		...
		#output = output.reshape(-1, self.dance_types)
		#return output

class C3D_Classifier(nn.Module):
	def __init__(self, frame_len=16, dance_types=16, train_whole=False):
		super(C3D_Classifier, self).__init__()
		c3d_model = C3D_lib.C3D()
		weights_path = os.path.join(os.getcwd(), 'models/C3D_model/c3d.pickle')
		trained_weights = torch.load(weights_path)
		c3d_model.load_state_dict(trained_weights)
		for p in c3d_model.parameters():
			p.requires_grad=train_whole
		modules_list = list(c3d_model.children())[:-2] 
		modules_list = modules_list[0:13] + [nn.Flatten()] + modules_list[13:]
		self.modules_list = []
		for m in modules_list:
			if (isinstance(m, nn.Conv3d) or isinstance(m, nn.Linear)):
				...
				self.modules_list.append(m)
		lin_layer = nn.Linear(4096, 16)
		for p in lin_layer.parameters():
			p.requires_grad=True
		self.modules_list.append(lin_layer)
...
class Vanilla_Classifier(nn.Module):
	def __init__(self, frame_len=32, dance_types=16, freeze_backbone=True):
		super(Vanilla_Classifier, self).__init__()
		self.freeze_backbone = freeze_backbone
		self.c3d_backbone = C3D_model.C3D()
		weights_path = os.path.join(os.getcwd(), 'models/C3D_model/c3d.pickle')
		trained_weights = torch.load(weights_path)
		self.c3d_backbone.load_state_dict(trained_weights)
		self.relu = nn.ReLU()
		self.dropout = nn.Dropout(p=0.4)
		self.classifier = nn.Linear(487,dance_types)
	def forward(self, x):
		if (self.freeze_backbone):
			with torch.no_grad():
				x = self.c3d_backbone(x)
		else:
			x = self.c3d_backbone(x)
		#print(x.shape)
		x = self.relu(x)
...
class Dance_classifier(nn.Module):
	def __init__(self, frame_len=16, heatmap_size=48, dance_types=16, trained_checkpoint=None, feature_extractor ):
		super(Dance_classifier, self).__init__()
		self.heatmaps_model = bodypose_model()
		if trained_checkpoint:
			checkpoint = torch.load(trained_checkpoint)
			self.heatmaps_model.load_state_dict(checkpoint['model_state_dict'])
		for p in self.heatmaps_model.parameters():
			p.requires_grad = False
		self.linear1 = nn.Linear(heatmap_size ** 2 * 2 * frame_len, 512)
		self.classifier = nn.Linear(512, dance_types)
		#self.conv1 = nn.Conv3d(64, 16, (1, 3, 3), stride=(1, 1, 1)) #convolving along the frame dimension
		#self.conv2 = nn.Conv3d(16, 4, (1, 3, 3), stride=(1,1,1)) #convolving more along the frame dimension
		#self.linear1 = nn.Linear((heatmap_size-4)**2 * 2, 768) #dense layer that extracts features from spatial pafs and confs 
		#self.linear2 = nn.Linear(768 * 4, 512) #dimension reduction
		#self.linear3 = nn.Linear(512,128) #dimension reduction
		#self.classifier = nn.Linear(128, dance_types) # classification layer
	def forward(self, x):
		with torch.no_grad():
			heatmaps_b = []
			for t in range(x.shape[0]):
				heatmaps_b.append(self.heatmaps_model.produce_maxed_heatmaps(x[t]))
			x = torch.stack(heatmaps_b)
		x = x.view(x.size(0), -1)
		x = self.linear1(x)
		x = F.relu(x)
...
class features_extractor(nn.Module):
	...
	def __init__(self):
		...
		self.all_features = models.vgg19(pretrained = True).features[0:23]
...
class new_bodypose_model(nn.Module):
	def __init__(self, paf_stages, conf_stages, batch_norm, final_activations):
...
class bodypose_model(nn.Module):
	def __init__(self, trained_model=True, n_stages=6):
	...
	def forward(self, x):
		...
		out1_1 = self.model1_1(out1)
		out1_2 = self.model1_2(out1)
		inp2 = torch.cat([out1, out1_1, out1_2], 1)
		#out2 = torch.cat([out1_1, out1_2], 1)
		out2_1 = self.model2_1(inp2)
		out2_2 = self.model2_2(inp2)
		inp3 = torch.cat([out1, out2_1, out2_2], 1)
		#out3 = torch.cat([out2_1, out2_2], 1)
		out3_1 = self.model3_1(inp3)
		out3_2 = self.model3_2(inp3)
		inp4 = torch.cat([out1, out3_1, out3_2], 1)
		#out4 = torch.cat([out3_1, out3_2], 1)
		out4_1 = self.model4_1(inp4)
		out4_2 = self.model4_2(inp4)
		inp5 = torch.cat([out1, out4_1, out4_2], 1)
		#out5 = torch.cat([out4_1, out4_2], 1)
		out5_1 = self.model5_1(inp5)
		out5_2 = self.model5_2(inp5)
		inp6 = torch.cat([out1, out5_1, out5_2], 1)
		#out6 = torch.cat([out5_1, out5_2], 1)
		out6_1 = self.model6_1(inp6)
		out6_2 = self.model6_2(inp6)
		#inp7 = torch.cat([out1, out6_1, out6_2], 1)
		paf_out = torch.stack([out1_1, out2_1, out3_1, out4_1, out5_1, out6_1])
		conf_out = torch.stack([out1_2, out2_2, out3_2, out4_2, out5_2, out6_2])
		return paf_out, conf_out
	"""
